...
def generate_gpmesh_json():
    ...
    gpmesh = {
        "version": 1,
        # MUDANÇA 1: Alterar o formato para PosNormTex (sem Skin)
        "vertexformat": "PosNormTex", 
        # MUDANÇA 2: Alterar o shader para BasicMesh (ou Padrão) ao invés de Skinned
        "shader": "BasicMesh", 
        "textures": [],
        "specularPower": 100.0,
        "vertices": [],
    ...
    for vert in mesh.vertices:
        ...
        normal = vert.normal
        gp_vert = []
        # Adiciona Posição (3 floats)
        gp_vert.extend([pos.y, pos.x, pos.z])
        # Adiciona Normal (3 floats)
        gp_vert.extend([normal.y, normal.x, normal.z])

        # MUDANÇA 3: Removemos todo o bloco de cálculo de ossos (boneIndices e weights)
        # que adicionava os 8 floats extras.
        
        gpmesh["vertices"].append(gp_vert)
    
    # UVs são armazenados separadamente
    for l in mesh.loops:
